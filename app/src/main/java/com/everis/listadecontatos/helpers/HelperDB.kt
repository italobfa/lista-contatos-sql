...
class HelperDB(
    ...
    fun buscarContatos(busca: String, isBuscaPorId: Boolean = false): List<ContatosVO>{
        ...
        //val cursor = db.rawQuery(sql, arrayOf(buscaComPercentual))
        if(cursor == null){
            db.close()
    ...
    fun deletarContato (id: Int){
        ...
        //val argss = arrayOf<String>("$id")
        //db.execSQL(sql, argss)
        val where = "id = ?"
        val args = arrayOf<String>("$id")
    ...
    fun updateContato (contato: ContatosVO){
        val db = writableDatabase ?: return
        //val sql = "UPDATE $TABLE_NAME $COLUMNS_NOME = ?, $COLUMNS_TELEFONE = ? WHERE $COLUMNS_ID = ?"
        //val argss = arrayOf<Any>(contato.nome, contato.telefone, contato.id)
        //db.execSQL(sql, argss)
        val content = ContentValues()
        content.put(COLUMNS_NOME, contato.nome)
        content.put(COLUMNS_TELEFONE, contato.telefone)
        val where = "id = ?"
        val args = arrayOf<String>("${contato.id}")
